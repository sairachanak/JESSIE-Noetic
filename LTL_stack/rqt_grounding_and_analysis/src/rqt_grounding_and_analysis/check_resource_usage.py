...
def get_prop_dot_name_real_name_mapping(nodes_dict):
    ...
    for dot_name, dot_name_properties in nodes_dict.items():
...
def check_left_behind_robot_per_prop(output_published_topics, robot_list, topic_filtered_list = []):
    ...
    for output_prop, published_topics_list in output_published_topics.items():
    ...
    for output_prop, topic_to_robot_dict in props_to_robots.items():
        for topic, robot_info in topic_to_robot_dict.items():
    ...
    for topic, topic_robot_dict in robot_topic_usage.items():
...
def check_possible_action_affected_sensors_old(subscribed_topics, output_published_topics):
    ...
    for prop, prop_info_list in subscribed_topics.items():
...
if __name__ == "__main__":
    ...
    for subgraph_name, subgraph in dot_file.obj_dict['subgraphs'].items():
        ...
        # add in nodes
        for node_id, node in subgraph[0]['nodes'].items():
            #if len(node) != 1:
        ...
        # add in edges:
        for edge_id, edge in subgraph[0]['edges'].items():
