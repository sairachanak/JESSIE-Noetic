...
def create_launch_file(launch_file, example_name, input_prop_to_ros_info, output_prop_to_ros_info):
    with open(launch_file, 'w+') as f_launch_file:
        ...
        for prop, prop_info in input_prop_to_ros_info.items():
            # check if parameters exist
            if 'parameters' in prop_info.keys():
                args_opt = " ".join(["--{key} {value}".format(key=key, value=value) for key, value in prop_info['parameters'].items()])
            else:
                args_opt = ""
        ...
        for prop, prop_info in output_prop_to_ros_info.items():
            # check if parameters exist
            if 'parameters' in prop_info.keys():
                args_opt = " ".join(["--{key} {value}".format(key=key, value=value) for key, value in prop_info['parameters'].items()])
            else:
    ...
    if not namespaces_to_node_name[current_namespace]:
        ...
        if prop in input_prop_to_ros_info.keys(): # input
            ...
            if 'parameters' in input_prop_to_ros_info[prop].keys():
                args_opt = " ".join(["--{key} {value}".format(key=key, value=value) for key, value in input_prop_to_ros_info[prop]['parameters'].items()])
            else:
                args_opt = ""
